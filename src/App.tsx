...
import { causeLabel, tr, type Language } from './lib/i18n';
...
function EmptyState({ onSelect, onDemo, onLanguage, onTheme, language, theme }: { onSelect: () => void; onDemo: () => void; onLanguage: () => void; onTheme: () => void; language: Language; theme: AppState['settings']['theme'] }) {
  return (
    <div className="empty-state" data-theme={theme}>
      ...
      <div className="empty-stone"><span>NO PROJECTS<br />REST HERE</span></div>
      ...
      <div className="privacy-strip"><span>●</span> 完全离线 <span>●</span> 用户主动授权 <span>●</span> 仅本地保存</div>
    </div>
  );
}
  ...
  return (
    <div className="project-table-wrap">
      <table className="project-table">
        <thead><tr><th>{tr(language, 'table.project')}</th><th>{tr(language, 'table.status')}</th><th>{tr(language, 'table.timeline')}</th><th>{tr(language, 'table.tech')}</th><th>{tr(language, 'table.activity')}</th><th>{tr(language, 'table.sleep')}</th><th>{tr(language, 'table.size')}</th><th>TODO</th><th>{tr(language, 'table.git')}</th></tr></thead>
        <tbody>
          {projects.map((project) => (
            <tr key={project.id} onClick={() => onSelect(project)} tabIndex={0} onKeyDown={(event) => event.key === 'Enter' && onSelect(project)}>
              <td><strong>{project.name}</strong><small>{project.path}</small></td>
              <td><StatusPill status={project.status} language={language} /></td>
              <td className="timeline-cell"><span><i>{tr(language, 'time.born')}</i>{formatDate(project.bornAt)}</span><span><i>{tr(language, 'time.death')}</i>{project.death ? formatDate(project.death.date) : tr(language, 'time.pending')}</span></td>
              <td><div className="tech-list">{project.technologies.slice(0, 3).map((tech) => <span key={tech}>{tech}</span>)}</div></td>
              <td>{formatDate(project.lastActiveAt)}</td><td>{project.sleepingDays} {tr(language, 'common.days')}</td><td>{formatBytes(project.sizeBytes)}</td><td>{project.todoCount}</td>
              <td><span className={`git-state ${project.gitState}`}>{project.gitState}</span></td>
            </tr>
          ))}
        </tbody>
      </table>
      {projects.length === 0 && <div className="no-results">这里没有符合筛选条件的项目。</div>}
    </div>
  );
}
      ...
      <aside className="detail-drawer">
        ...
        {project.death && (
          <div className="memorial-card">
            <span>IN MEMORY OF</span><div className="memorial-dates"><p><i>{tr(language, 'time.born')}</i>{formatDate(project.bornAt)}</p><em>→</em><p><i>{tr(language, 'time.death')}</i>{formatDate(project.death.date)}</p></div>
  ...
  return (
    <div className="modal-backdrop">
      <form className="modal funeral-modal" onSubmit={submit}>
        ...
        <p className="eyebrow">{tr(language, 'funeral.record')}</p><h2>{tr(language, 'funeral.for')} {project.name}</h2>
        <p className="modal-copy">{tr(language, 'funeral.copy')}</p>
...
function ReportModal({ state, language, onClose }: { state: AppState; language: Language; onClose: () => void }) {
  ...
  return (
    <div className="modal-backdrop"><div className="modal report-modal">
      ...
      <p className="eyebrow">ANNUAL MEMORIAL</p>
...
export default function App() {
  ...
  async function archive(project: GraveProject) {
    if (!window.confirm(`确认开始移动目录流程？\n\n${project.path}\n\n下一步仍需选择归档目录并通过系统确认。`)) return;
    if (state.settings.demoMode || !window.graveyard) { setMessage('演示模式不会移动真实目录。'); return; }
  ...
  async function trash(project: GraveProject) {
    if (!window.confirm(`确认开始回收流程？\n\n${project.path}\n\n项目目录只会移入系统回收站，不会永久删除。下一步仍需系统确认。`)) return;
    if (state.settings.demoMode || !window.graveyard) { setMessage('演示模式不会操作真实文件。'); return; }
  ...
  return (
    <div className="app-shell" data-theme={state.settings.theme} lang={language}>
